...
public class CategoryServiceImpl implements CategoryService {
    ...
    public void delete(Integer id) {
        ...
        Example example = new Example(Category.class); // 传入需要查询的实体类
        Example.Criteria criteria = example.createCriteria() ; // 创建查询条件类
        criteria.andEqualTo("parentId",id); // 判断有没有父级id为当前分类的id
        int count = this.categoryMapper.selectCountByExample(example); // 查询
        if (count>0){ // 存在下级分类
            throw new RuntimeException("存在下级分类不能删除！"); // 创建一个运行时异常，最后会有springMvc，异常处理类来处理
        }
