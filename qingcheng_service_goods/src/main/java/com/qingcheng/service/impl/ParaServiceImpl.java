...
public class ParaServiceImpl implements ParaService {
    ...
    public void add(Para para) {
        Template template = this.templateMapper.selectByPrimaryKey(para.getTemplateId());// 查询出对应template数据
        template.setParaNum(template.getParaNum()+1); // 数据加1
        this.templateMapper.updateByPrimaryKeySelective(template); // 跟新数据
    ...
    public void delete(Integer id) {
        Para para = this.paraMapper.selectByPrimaryKey(id); // 查询 para
        Template template = this.templateMapper.selectByPrimaryKey(para.getTemplateId()); // 查询para对应的模版表数据

        template.setParaNum(template.getParaNum()-1); // 数据减1
        this.templateMapper.updateByPrimaryKeySelective(template); // 更数据

        paraMapper.deleteByPrimaryKey(id); // 删除数据
    }
    ...
    public void deleteList (Map<String,Object> searchMap) {
        ...
        this.paraMapper.deleteByExample(example);
    }
